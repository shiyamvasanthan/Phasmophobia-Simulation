...
Purpose: Initialize room list, evidence list, and init the mutex of the shared evidence list                        
in: The building
out: The initalized building */
void initBuilding(BuildingType* b){
	initRoomList(&b->rooms);
	initEvidenceList(&b->sharedEvidence);
	sem_init(&(b->sharedEvidence.mutex), 0, 1);
}

/*
Function: cleanupBuilding    
Purpose: Cleans up the nodes of the hunter's evidence list, cleans up all the rooms and it's evidence, cleans up the shared evidence list                    
in: The building
out: The cleaned up building */
void cleanupBuilding(BuildingType* b){
	for(int i = 0; i < MAX_HUNTERS; i++){
		cleanupEvidenceNodes(b->hunters[i].evidence);
		free(b->hunters[i].evidence);
	}
	
	cleanupConnectedRooms(&b->rooms);
	cleanupAllRooms(&b->rooms);
	cleanupEvidenceList(&b->sharedEvidence);
}

/*
Function: populateRooms    
Purpose: Populates the building with rooms and connects them togethers                  
in: The building
out: The building */
void populateRooms(BuildingType* building) {
	...
	//Changed to pass in RoomListType*
